<template>
<uni-shadow-root class="TUICalling-TUICalling"><view :class="'TUICalling '+(callStatus === 'idle' ? 'hidden': 'show')">
  <view class="TRTCCaling-container">
    <TUI-Calling v-if="callStatus === 'calling'" :isSponsor="isSponsor" :pusher="pusher" :callType="config.type" :remoteUsers="remoteUsers" @callingEvent="handleCallingEvent"></TUI-Calling>
    <TUI-Connected v-if="callStatus === 'connection'" :playerList="playerList" :pusher="pusher" :callType="config.type" :soundMode="soundMode" :screen="screen" @connectedEvent="handleConnectedEvent"></TUI-Connected>
  </view>
...
global['__wxVueOptions'] = {components:{'TUI-Calling': TUICalling,'TUI-Connected': TUIConnected}}
