<template>
	<view class="container">
		...
		<view class="input-container">
			<label class="list-item-label" style="margin-bottom: 10px;">呼叫用户ID:<span style="color:#C0C0C0;margin-left: 8px;">user1;user2(“;”为英文分号)</span></label>
			<input  class="input-box" v-model="callUserID"  maxlength="140" type="text" placeholder="输入userID" placeholder-style="color:#BBBBBB;">
		</view>
	  <view class="guide-box">
		  <view class="single-box" :id="index" @click="callingHandle(item.type)" v-for="(item, index) in entryInfos" :key="index">
			...
			uni.$TUIKit.getUserProfile({
				 ...
				 .then((res) => {
					 ...
					 if(res.data.length == 1) {
						 this.$refs.TUICalling.call({userID: userIDs[0], type})
					 }
					 if(res.data.length > 1) {
						 this.$refs.TUICalling.groupCall({userIDList: userIDs, type: type})
					 } 
				 })
				 .catch(()=> {
					 uni.showToast({
