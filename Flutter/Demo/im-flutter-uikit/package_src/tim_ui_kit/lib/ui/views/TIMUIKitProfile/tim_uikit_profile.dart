...
class TIMUIKitProfile extends StatefulWidget {
  ...
  final Widget Function(BuildContext context, V2TimFriendInfo friendInfo,
      V2TimConversation conversation, int friendType)? operationListBuilder;
  ...
  /// 备注
  static Widget remarkBar(String remark, BuildContext context, {Function()? handleTap}) {
  ...
  /// 加入黑名单
  static Widget addToBlackListBar(bool value, BuildContext context, Function(bool value)? onChanged) {
    final I18nUtils ttBuild = I18nUtils(context);
...
class _TIMUIKitProfileState extends State<TIMUIKitProfile> {
  ...
  handleTapRemarkBar(BuildContext context) {
    final I18nUtils ttBuild = I18nUtils(context);
    _controller.showTextInputBottomSheet(context, ttBuild.imt("修改备注名"), ttBuild.imt("仅限中字、字母、数字和下划线"),
        (String remark) {
  ...
  Widget _defaultOperationListBuilder(V2TimFriendInfo friendInfo,
      V2TimConversation conversation, bool isFriend) {
    final I18nUtils ttBuild = I18nUtils(context);
    return isFriend
        ? Column(
            children: [
              TIMUIKitProfile.operationDivider(),
              TIMUIKitProfile.remarkBar(friendInfo.friendRemark ?? ttBuild.imt("无"),context,
                  handleTap: handleTapRemarkBar(context)),
              TIMUIKitProfile.operationDivider(),
              TIMUIKitProfile.pinConversationBar(conversation.isPinned ?? false,context,
                  (value) {
          ...
          return Column(children: [
                  ...
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) => TIMUIKitProfilePersonalInfoPage(
                              userID: widget.userID, appBarTile: ttBuild.imt("个人资料"),
                            )),
                  );
                }
            ...
            widget.operationListBuilder != null
                ...
                : _defaultOperationListBuilder(
                    userInfo, conversation, isFriend),
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider.value(
        ...
        child: Consumer<TUIThemeViewModel>(builder: (context, value, child) {
          ...
          final signatureText =
              signature != null ? ttBuild.imt_para("个性签名: {{signature}}", "个性签名: ${signature}")(signature: signature) : ttBuild.imt("暂无个性签名");
