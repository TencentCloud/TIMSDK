...
class TIMUIKitSearchMsg extends StatelessWidget {
  ...
  TIMUIKitSearchMsg(
      {required this.msgList,
        ...
        required this.totalMsgCount, Key? key, required this.onTapConversation})
      : super(key: key);

  Widget _renderShowALl(bool isShowMore, I18nUtils ttBuild){
    return (isShowMore == true) ? TIMUIKitSearchShowALl(
      textShow: ttBuild.imt("更多聊天记录"),
      onClick: () => {
        model.searchMsgByKey(keyword, false)
      },
    ) : Container();
  }
              ...
              return TIMUIKitSearchItem(
                onClick: () async {
                  await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => TIMUIKitSearchMsgDetail(
                ...
                lineTwo: ttBuild.imt_para("{{count}}条相关聊天记录","$count条相关聊天记录")(count: count),
              );
            }).toList(),
