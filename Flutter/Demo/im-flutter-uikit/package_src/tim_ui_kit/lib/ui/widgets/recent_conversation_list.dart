...
class RecentForwardList extends StatefulWidget {
  ...
  const RecentForwardList({Key? key, this.isMultiSelect = true, this.onChanged})
      : super(key: key);
...
class _RecentForwardListState extends State<RecentForwardList> {
  ...
  Widget build(BuildContext context) {
    ...
    final recentConvList = _conversationViewModel.conversationList;
    final showList = _buildMemberList(recentConvList);
    return ChangeNotifierProvider.value(
        value: serviceLocator<TUIThemeViewModel>(),
        child: Consumer<TUIThemeViewModel>(
          builder: (context, value, child) => AZListViewContainer(
            memberList: showList,
            ...
            susItemBuilder: (context, index) {
              return Container(
                ...
                padding: const EdgeInsets.only(left: 16.0),
                color: value.theme.weakDividerColor,
                alignment: Alignment.centerLeft,
                child: Text(
                  ttBuild.imt("最近联系人"),
                  softWrap: true,
