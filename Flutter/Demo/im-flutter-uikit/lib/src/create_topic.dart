...
class CreateTopicState extends State<CreateTopic> {
  ...
  getMessages() async {
    if (widget.messageIdList.isNotEmpty) {
      ...
      if (res.code == 0) {
        ...
        Utils.log(imt_para("获取到的消息:{{getMsg}}", "获取到的消息:${getMsg}")(getMsg: getMsg));
        setState(() {
  ...
  getDiscussList() async {
    ...
    if (code == 0) {
      ...
      Utils.toast(imt_para("获取讨论区列表失败 {{message}}", "获取讨论区列表失败 ${message}")(message: message));
    }
  }
  ...
  createTopic() async {
    ...
    if (userId!.isNotEmpty) {
      ...
      Utils.log(data);
      if (code != 0) {
        Utils.toast(imt_para("创建话题失败 {{message}}", "创建话题失败 ${message}")(message: message));
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        shadowColor: CommonColor.weakBackgroundColor,
        elevation: 1,
        title: Text(
          imt("创建话题"),
          style: TextStyle(color: Colors.black),
        ),
        backgroundColor: CommonColor.weakBackgroundColor,
      ),
        ...
        child: Column(
          children: [
            if (messageList.isNotEmpty)
              Text(
                imt("---- 相关讨论 ----"),
                style: TextStyle(color: CommonColor.weakTextColor),
              ),
              ...
              Container(
                ...
                child: InkWell(
                  child: Text(
                    ...
                    style: TextStyle(
                      color:
                          Color(int.parse('006eff', radix: 16)).withAlpha(255),
                      fontSize: 12,
                    ),
                  ),
                ),
