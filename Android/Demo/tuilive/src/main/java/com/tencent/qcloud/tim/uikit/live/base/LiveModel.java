...
public class LiveModel implements Cloneable, Serializable {
    ...
    public static LiveModel convert2LiveData(V2TIMMessage msg) {
        ...
        liveModel.message = TUILiveService.getAppContext().getString(R.string.invalid_command);
        try {
            ...
            switch (code) {
                case CODE_REQUEST_JOIN_ANCHOR:
                    liveModel.message = TUILiveService.getAppContext().getString(R.string.request_online);
                    break;
                case CODE_RESPONSE_JOIN_ANCHOR:
                    if (actionType == V2TIMSignalingInfo.SIGNALING_ACTION_TYPE_REJECT_INVITE) {
                        liveModel.message = TUILiveService.getAppContext().getString(R.string.reject_request_online);
                    } else if (actionType == V2TIMSignalingInfo.SIGNALING_ACTION_TYPE_ACCEPT_INVITE) {
                        liveModel.message = TUILiveService.getAppContext().getString(R.string.agree_request_online);
                    }
                    break;
                case CODE_KICK_OUT_JOIN_ANCHOR:
                    liveModel.message = TUILiveService.getAppContext().getString(R.string.request_close_online);
                    break;
                case CODE_RESPONSE_KICK_OUT_JOIN_ANCHOR:
                    liveModel.message = TUILiveService.getAppContext().getString(R.string.close_online);
                    break;
                case CODE_REQUEST_ROOM_PK:
                    if (actionType == V2TIMSignalingInfo.SIGNALING_ACTION_TYPE_INVITE) {
                        liveModel.message = TUILiveService.getAppContext().getString(R.string.request_pk);
                    }
                    break;
                case CODE_RESPONSE_PK:
                    if (actionType == V2TIMSignalingInfo.SIGNALING_ACTION_TYPE_REJECT_INVITE) {
                        liveModel.message = TUILiveService.getAppContext().getString(R.string.reject_request_pk);
                    } else if (actionType == V2TIMSignalingInfo.SIGNALING_ACTION_TYPE_ACCEPT_INVITE) {
                        liveModel.message = TUILiveService.getAppContext().getString(R.string.agree_request_pk);
                    }
                    break;
                case CODE_QUIT_ROOM_PK:
                    liveModel.message = TUILiveService.getAppContext().getString(R.string.exit_pk);
                    break;
            }
        } catch (Exception e) {
            TUILiveLog.e(TAG, "convert2LiveData exception:" + e);
        }
        return liveModel;
    }
    ...
    public static boolean isLiveRoomSignal(String data) {
        ...
        try {
            ...
            TUILiveLog.e(TAG, "isLiveRoomSignal exception:" + e);
        }
        return false;
    }
