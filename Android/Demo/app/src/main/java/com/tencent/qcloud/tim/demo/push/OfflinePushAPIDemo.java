...
public class OfflinePushAPIDemo {
     ...
     *
     * When the IM console sets the click follow-up action to "use push component callback to jump"，After the push is successful, register NotifyEvent through TUICore, and click the notification bar event will be returned through the TUICore.onNotifyEvent callback.
     */
    ...
    /*
     * 当 IM 控制台设置点击后续动作为 "使用推送组件回调跳转"，推送成功后，注册广播接收者，点击通知栏事件通过发送广播回调返回，IntentFilter 为 TUIConstants.TUIOfflinePush.NOTIFICATION_BROADCAST_ACTION。
     *
     * When the IM console sets the click follow-up action to "use push component callback to jump"，After the push is successful, register the broadcast receiver, click the notification bar event to return by sending the broadcast callback.
     * IntentFilter 为 TUIConstants.TUIOfflinePush.NOTIFICATION_BROADCAST_ACTION.
     */
