    ...
    private void initView(Context context) {
        ...
        mRadioGroupUserListType.setOnCheckedChangeListener((group, checkedId) -> {
            if (checkedId == mBtnOnSeatTab.getId()) {
                ConferenceController.sharedInstance().getViewController().updateUserTypeSelected(ViewState.UserListType.ON_SEAT_INSIDE_THE_ROOM);
            } else if (checkedId == mBtnOffSeatTab.getId()) {
                ConferenceController.sharedInstance().getViewController().updateUserTypeSelected(ViewState.UserListType.OFF_SEAT_INSIDE_THE_ROOM);
            } else if (checkedId == mBtnEnteredTab.getId()) {
                ConferenceController.sharedInstance().getViewController().updateUserTypeSelected(ViewState.UserListType.ALL_USER_ENTERED_THE_ROOM);
            } else if (checkedId == mBtnNotEnteredTab.getId()) {
                ConferenceController.sharedInstance().getViewController().updateUserTypeSelected(ViewState.UserListType.ALL_USER_NOT_ENTERED_THE_ROOM);
            }
        });
    }
