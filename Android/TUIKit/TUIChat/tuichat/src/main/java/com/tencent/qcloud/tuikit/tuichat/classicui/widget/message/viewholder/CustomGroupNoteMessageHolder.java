...
public class CustomGroupNoteMessageHolder extends MessageContentHolder{
    public CustomGroupNoteMessageHolder(View itemView) {
        super(itemView);
    }

    public static final String TAG = CustomGroupNoteMessageHolder.class.getSimpleName();

    @Override
    public int getVariableLayout() {
        Object resIDObject = TUICore.callService(TUIConstants.TUIGroupNote.SERVICE_NAME, TUIConstants.TUIGroupNote.METHOD_GET_GROUP_NOTE_MESSAGE_LAYOUT, null);
        int resID = 0;
        if (resIDObject != null) {
    ...
    public void layoutVariableViews(TUIMessageBean msg, int position) {
        ...
        if (msg != null && msg instanceof CustomGroupNoteMessageBean) {
            CustomGroupNoteMessageBean customGroupNoteMessageBean = (CustomGroupNoteMessageBean) msg;
            CustomGroupNoteMessageBean.CustomGroupNoteMessage customGroupNoteMessage = customGroupNoteMessageBean.getCustomGroupNoteMessage();
            ...
            param.put(TUIConstants.TUIChat.PLUGIN_BEAN_OBJECT, ((CustomGroupNoteMessageBean) msg).getGroupNoteBeanObject());
            IUIKitCallback<Object> pluginCallback = new IUIKitCallback<Object>() {
                @Override
                public void onSuccess(Object pluginObject) {
                    if (pluginObject != null) {
                        ((CustomGroupNoteMessageBean) msg).setGroupNoteBeanObject(pluginObject);
                    }
                }
            };
            param.put(TUIConstants.TUIChat.CALL_BACK, pluginCallback);
            TUICore.notifyEvent(TUIConstants.TUIGroupNote.EVENT_KEY_GROUP_NOTE_MESSAGE_LAYOUT, TUIConstants.TUIGroupNote.EVENT_SUB_KEY_REFRESH_GROUP_NOTE_MESSAGE_LAYOUT, param);
        }
    }
}
