...
object ImageLoader {
    ...
    fun load(context: Context, target: ImageView?, source: Any?, placeImage: Int) {
        val imageOptions = ImageOptions.Builder().setPlaceImage(placeImage).build()
    ...
    fun loadGif(context: Context, target: ImageView?, @RawRes @DrawableRes resourceId: Int) {
        ...
        Glide.with(context.applicationContext).asGif().load(resourceId).into(target)
    }
    ...
    fun transformBitmap(
        ...
        val builder = Glide.with(context.applicationContext).asBitmap().load(source)
        setBuilderOptions(context.applicationContext, builder, options)
        return try {
            builder.submit(width, height).get() as? Bitmap
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
    ...
    private fun setBuilderOptions(
        ...
        if (config.roundRadius > 0) {
            transformations.add(RoundedCorners(dip2px(config.roundRadius.toFloat())))
        }
        ...
        options.diskCacheStrategy(if (config.skipDiskCache) DiskCacheStrategy.NONE else DiskCacheStrategy.ALL)
        options.skipMemoryCache(config.skipMemoryCache)
        builder.apply(options)
    }

    class BlurTransformation(context: Context, level: Float) : BitmapTransformation() {
        private val radius: Float = level * 0.25f
        private val weakPreference: WeakReference<Context> = WeakReference(context)
        ...
        override fun transform(
            ...
            return BlurUtils.blur(weakPreference.get()!!, pool, toTransform, radius.toInt())
        }
    }
}
