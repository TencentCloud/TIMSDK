...
## 最新增强版 8.7.7201 @2025.09.05
### SDK
- 网络长连接通道支持标准 TLS 协议
- 网络长连接 DNS 地址增加 cn 备份域名
- 网络长连接增加 Http 类型的内置 IP 地址
- 支持 RoomEngine 同步查询商业化能力位
- 支持 RoomEngine 拉取加入直播间前 20 条消息
- Push 支持 OPPO 私信通道推送模版
- Push 支持 FCM 消息优先级设置
- Push 支持 vivo 通知类型设置
- 增加“同时加入多个直播群”的数据上报
- 鸿蒙平台 IMSDK 支持构建 C++ 接口
- 支持向社群发送群定向消息
- 支持登录失败后插入本地消息
- 支持创建Work工作群时设置加群审批选项
- Unity SDK 补齐云端搜索 API
- 优化 APP 在被踢下线后发消息的错误信息描述
- 拉取直播群群成员列表增加云控限频
- 支持 iOS/Mac 平台在 SDK 线程触发回调
- 修复安卓平台 Zip 解压安全漏洞
