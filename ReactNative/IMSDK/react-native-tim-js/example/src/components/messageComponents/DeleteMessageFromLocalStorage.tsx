...
const DeleteMessageFromLocalCStorageComponent = () => {
    const [res, setRes] = useState<any>({});
    const [conversationID, setConversationID] = useState<string>('未选择')
    const [message, setMessage] = useState<string>('[]')

    const deleteMessageFromLocalCStorage= async () => {
        const res = await TencentImSDKPlugin.v2TIMManager.getMessageManager().deleteMessageFromLocalStorage(message)
        setRes(res)
    };
    ...
    return (
        <View style={{height: '100%'}}>
            ...
            <CommonButton
                handler={() => { deleteMessageFromLocalCStorage() }}
                content={'删除本地信息'}
            ></CommonButton>
...
export default DeleteMessageFromLocalCStorageComponent;
