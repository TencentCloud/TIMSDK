...
const SetLocalCustomIntComponent = () => {
    const [res, setRes] = useState<any>({});
    const [conversationID, setConversationID] = useState<string>('未选择')
    const [message, setMessage] = useState<string>('[]')

    const setLocalCustomInt = async () => {
        const res = await TencentImSDKPlugin.v2TIMManager.getMessageManager().setLocalCustomInt(message,0)
        setRes(res)
    };
...
export default SetLocalCustomIntComponent;
