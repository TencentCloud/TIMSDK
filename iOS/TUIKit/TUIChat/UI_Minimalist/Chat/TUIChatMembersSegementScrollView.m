...
- (instancetype)initWithFrame:(CGRect)frame SegementItems:(NSArray<TUIChatMembersSegementItem *> *)items block:(btnClickedBlock)clickedBlock {
    ...
    if (self) {
        ...
        for (int i = 0; i < titleCount; i++) {
            ...
            if (preBtn == nil) {
                btn = [[TUIFitButton alloc]
                    initWithFrame:CGRectMake(0, (self.frame.size.height - SEGEMENT_BTN_HEIGHT) * 0.5, SEGEMENT_BTN_WIDTH, SEGEMENT_BTN_HEIGHT)];
            } else {
                btn = [[TUIFitButton alloc]
                    initWithFrame:CGRectMake(preBtn.frame.origin.x + preBtn.frame.size.width + padding, (self.frame.size.height - SEGEMENT_BTN_HEIGHT) * 0.5,
                                             SEGEMENT_BTN_WIDTH, SEGEMENT_BTN_HEIGHT)];
            }
            preBtn = btn;
        ...
        self.selectedLine.frame =
            CGRectMake(currentBtn.frame.origin.x, (self.frame.size.height - SEGEMENT_BTN_HEIGHT) * 0.5, SEGEMENT_BTN_WIDTH, SEGEMENT_BTN_HEIGHT);
...
- (TUIChatMembersSegementView *)mySegementView {
    if (_mySegementView == nil) {
        _mySegementView = [[TUIChatMembersSegementView alloc] initWithFrame:CGRectMake(kScale390(27), 0, SCROLLVIEW_WIDTH - kScale390(54), SEGEMENT_HEIGTHT)
