...
- (void)loadOriginMessageFromReplyData:(TUIReplyMessageCellData_Minimalist *)replycellData dealCallback:(void(^)(void))callback
{
    ...
    @weakify(replycellData)
    [TUIChatDataProvider_Minimalist findMessages:@[replycellData.originMsgID] callback:^(BOOL succ, NSString * _Nonnull error_message, NSArray * _Nonnull msgs) {
        @strongify(replycellData)
        if (!succ) {
        ...
        if ([cellData isKindOfClass:TUIImageMessageCellData_Minimalist.class]) {
            TUIImageMessageCellData_Minimalist *imageData = (TUIImageMessageCellData_Minimalist *)cellData;
            ...
            TUIVideoMessageCellData_Minimalist *videoData = (TUIVideoMessageCellData_Minimalist *)cellData;
