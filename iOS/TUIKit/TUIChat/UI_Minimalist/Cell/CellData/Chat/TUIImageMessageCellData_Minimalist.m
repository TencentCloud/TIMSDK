...
+ (TUIMessageCellData *)getCellData:(V2TIMMessage *)message {
    ...
    TUIImageMessageCellData_Minimalist *imageData =
        [[TUIImageMessageCellData_Minimalist alloc] initWithDirection:(message.isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
...
- (CGSize)contentSize {
    ...
    BOOL isDir = NO;
    if (![self.path isEqualToString:@""] && [[NSFileManager defaultManager] fileExistsAtPath:self.path isDirectory:&isDir]) {
        if (!isDir) {
