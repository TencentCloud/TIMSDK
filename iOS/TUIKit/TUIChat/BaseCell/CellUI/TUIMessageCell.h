 ...
 *  XXXX消息单元（TUIXXXXMessageCell）主要负责在页面上的显示和用户交互事件的响应。消息单元内的数据处理与获取，请根据具体消息单元参考 TUIChat\CellData\TUIXXXXMessageCellData.h
 *
 ...
 *  The above messages are implemented by inheriting from this class or a subclass of this class. If you want to customize the message, you also need to implement it by inheriting from this class or a subclass of this class.
 *  The XXXX message unit (TUIXXXXMessageCell) is mainly responsible for displaying on the page and responding to user interaction events. For data processing and acquisition in the message unit, please refer to TUIChat\CellData\TUIXXXXMessageCellData.h according to the specific message unit
 *
 ...
 *  The TUIMessageCell class stores message-related information, such as the sender's avatar, sender's nickname, and message content (supports various formats such as text, pictures, and videos).
 *  At the same time, TUIMessageeCell, as a parent class, provides basic properties and behavior templates for subclass messages.
 */
 ...
 *  You can use this callback to implement secondary operations such as delete and recall (when the sender of the message long-presses his own message) on top of the long-pressed message.
 */
 ...
 *
 *  The message data class which stores the information required in the messageCell, including sender ID, sender avatar, message sending status, message bubble icon, etc.
 *  For details of messageData, please refer to: TUIChat\Cell\CellData\TUIMessageCellData.h
 */
