...
+ (void)configAvatarByParam:(NSDictionary *)param targetView:(UIImageView *)targetView {
    ...
    if (groupID.length > 0) {
            ...
            if (IS_NOT_EMPTY_NSSTRING(faceUrl)) {
                 ...
                 */
                [targetView sd_setImageWithURL:[NSURL URLWithString:faceUrl]
                                      placeholderImage:originAvatarImage];
            } else {
                /**
                ...
                if (TUIConfig.defaultConfig.enableGroupGridAvatar) {
                    ...
                    [targetView sd_setImageWithURL:nil
                                          placeholderImage:originAvatarImage];
                    [TUIGroupAvatar getCacheGroupAvatar:groupID callback:^(UIImage *avatar, NSString *groupID) {
                        @strongify(self)
                        if ([groupID isEqualToString:groupID]) {
                            ...
                            if (avatar != nil) {
                                ...
                                [targetView sd_setImageWithURL:nil
                                                      placeholderImage:originAvatarImage];
                                [TUIGroupAvatar fetchGroupAvatars:groupID placeholder:originAvatarImage callback:^(BOOL success, UIImage *image, NSString *groupID) {
                                    @strongify(self)
                                    if ([groupID isEqualToString:groupID]) {
                                        // callback 回调时，cell 未被复用
                                        // When the callback is invoked, the cell is not reused
                                        [targetView sd_setImageWithURL:nil placeholderImage:success?image:DefaultGroupAvatarImageByGroupType(groupType)];
                                    } else {
