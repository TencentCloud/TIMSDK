...
- (void)fillWithData:(TUIMessageCellData_Minimalist *)data {
    ...
    if (self.messageDataMini.direction == MsgDirectionOutgoing) {
        ...
        if (self.messageDataMini.status == Msg_Status_Sending || self.messageDataMini.status == Msg_Status_Sending_2) {
...
- (void)layoutSubviews {
    ...
    if (self.messageDataMini.messageModifyReacts.count > 0) {
        ...
        CGFloat emojiViewX = (self.messageDataMini.direction == MsgDirectionIncoming ? self.container.mm_x + kScale390(16)
