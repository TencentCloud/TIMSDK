...
- (void)setupView {
    ...
    self.themeSubLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.themeLabel.mm_maxX + kScale375(9), self.themeLabel.mm_y + 4, styleSubLabel.mm_w, styleSubLabel.mm_h)];
...
static UIWindow *im_window = nil;
- (void)showIMWindow {
    if (!im_window) {
        im_window = [[UIWindow alloc] initWithFrame:CGRectZero];
        im_window.windowLevel = UIWindowLevelAlert - 1;
        im_window.backgroundColor = [UIColor whiteColor];
        
        if (@available(iOS 13.0, *)) {
            for (UIWindowScene *windowScene in [UIApplication sharedApplication].connectedScenes) {
                if (windowScene.activationState == UISceneActivationStateForegroundActive) {
                    im_window.windowScene = windowScene;
                    break;
                }
            }
        }
        
        im_window.frame = CGRectMake(0, 0, Screen_Width, Screen_Height);
        im_window.rootViewController = [self getMainController];
        im_window.hidden = NO;
        ...
        [im_window addSubview:self.dismissWindowBtn];
    }
...
- (void)setupStyleWindow {
    if ([TUIStyleSelectViewController isClassicEntrance]) {
        ...
        self.dismissWindowBtn.hidden = YES;
        return;
    }
    ...
    [im_window bringSubviewToFront:self.dismissWindowBtn];
    ...
    [[[RACObserve(self.convVC_Mini, showLeftBarButtonItems) distinctUntilChanged] skip:1] subscribeNext:^(NSArray<UIBarButtonItem *>*showLeftBarButtonItems) {
        ...
        if (![im_window.subviews containsObject:titleItem.customView]) {
            titleItem.customView.mm_x = self.dismissWindowBtn.mm_x;
            titleItem.customView.mm_y = self.dismissWindowBtn.mm_maxY;
            [im_window addSubview:titleItem.customView];
        }
        [self.convVC_Mini.showLeftBarButtonItems removeAllObjects];
    }];
    
    [[[RACObserve(self.contactsVC_Mini, showLeftBarButtonItems) distinctUntilChanged] skip:1] subscribeNext:^(NSArray<UIBarButtonItem *>*showLeftBarButtonItems) {
    ...
    [[[RACObserve(self.settingVC_Mini, showLeftBarButtonItems) distinctUntilChanged] skip:1] subscribeNext:^(NSArray<UIBarButtonItem *>*showLeftBarButtonItems) {
    ...
    [[[RACObserve(self.convVC_Mini, showRightBarButtonItems) distinctUntilChanged] skip:1] subscribeNext:^(NSArray<UIBarButtonItem *>*showRightBarButtonItems) {
        self.convShowRightBarButtonItems = [showRightBarButtonItems copy];
        for (UIBarButtonItem *item in self.convShowRightBarButtonItems) {
            if (![im_window.subviews containsObject:item.customView]) {
                ...
                [im_window addSubview:item.customView];
            }
        }
    ...
    [[[RACObserve(self.contactsVC_Mini, showRightBarButtonItems) distinctUntilChanged] skip:1] subscribeNext:^(NSArray<UIBarButtonItem *>*showRightBarButtonItems) {
...
- (UITabBarController *)getMainController_Classic {
    ...
    convVC.viewWillAppear = ^(BOOL isAppear) {
        ...
        if ((self.inContactsVC || self.inSettingVC) && !isAppear) {
            return;
        }
    ...
    contactVC.viewWillAppear = ^(BOOL isAppear) {
        ...
        if ((self.inConversationVC || self.inSettingVC) && !isAppear) {
            return;
        }
        self.dismissWindowBtn.hidden = !isAppear;
    };
    ...
    setVC.viewWillAppear = ^(BOOL isAppear) {
        ...
        if ((self.inConversationVC || self.inContactsVC) && !isAppear) {
            return;
        }
        self.dismissWindowBtn.hidden = !isAppear;
    };
...
- (void)updateMarkUnreadCount:(NSNotification *) note {
    ...
    [V2TIMManager.sharedInstance getTotalUnreadMessageCount:^(UInt64 totalCount) {
        @strongify(self)
        NSInteger unreadCalculationResults = [self caculateRealResultAboutSDKTotalCount:totalCount markUnreadCount:markUnreadCount markHideUnreadCount:markHideUnreadCount];
...
- (void)onTotalUnreadMessageCountChanged:(UInt64) totalUnreadCount {
    NSInteger unreadCalculationResults = [self caculateRealResultAboutSDKTotalCount:totalUnreadCount markUnreadCount:_markUnreadCount markHideUnreadCount:_markHideUnreadCount];
    [self onTotalUnreadCountChanged:unreadCalculationResults];
}
