 ...
 *  - 如果您返回 nil，TUIBaseChatViewController 会认为该条消息非自定义消息，会将其按照普通消息的处理流程进行处理。
 *  - 如果您返回一个 TUIMessageCellData 类型的对象，TUIBaseChatViewController 会在随后触发的 onShowMessageData() 回调里传入您返回的 cellData 对象。
 *
 *  也就是说，onNewMessage() 负责让您甄别自己的个性化消息，而 onShowMessageData() 回调则负责让您展示这条个性化消息。
 *
 * <pre>
 *  - (TUIMessageCellData *)chatController:(TUIBaseChatViewController *)controller onNewMessage:(TIMMessage *)msg
 *  {
 ...
 *
 * <pre>
 *  - (TUIMessageCell *)chatController:(TUIBaseChatViewController *)controller onShowMessageData:(TUIMessageCellData *)data
 *  {
 ...
 */
- (NSArray <TUIInputMoreCellData *> *)chatController:(TUIBaseChatViewController *)chatController onRegisterMoreCell:(MoreCellPriority *)priority;
 ...
 *
 *  TUIBaseChatViewController 类用于实现聊天视图的总控制器，负责将聊天消息控制器（TUIMessageController）、信息输入控制器（TUIInputController）和更多视频进行统一控制。
 *
...
/**
 *  被委托类，负责实现并执行 TUIChatControllerListener 的委托函数
 */
...
/**
 *  是否展示视频通话按钮，如果集成了 TUICalling 组件，默认会展示
 */
@property (nonatomic) BOOL isEnableVideoCall;

/**
 *  是否展示音频通话按钮，如果集成了 TUICalling 组件，默认会展示
 */
@property (nonatomic) BOOL isEnableAudioCall;

/**
 *  是否展示群直播通话按钮，如果集成了 TUIKitLive 组件，默认会展示
 */
@property (nonatomic) BOOL isEnableLive;
